...
const run = async () => {
  ...
  for await (const file of walkMarkdown("_docs")) {
    ...
    for (const [key, supported] of Object.entries(SCHEMA_MARKERS)) {
      if (key in data && data[key] !== supported) {
        fileErrors.push(`${key} must be the integer ${supported}`);
      }
    }
