...
// AC-005 / DEC-005: schema markers are typed known fields, while unrelated
// metadata remains observable as a warning.
const runFrontmatterSchemaMarkerCases = async () => {
  const repoRoot = Deno.cwd();
  const temp = await Deno.makeTempDir({ prefix: "docs-dd-frontmatter-" });
  const document = (extra) =>
    `---
title: Schema marker fixture
status: active
draft_status: n/a
...
# Schema marker fixture
`;
  const frontMatter = (extra) => document(extra).split("\n# ")[0];

  try {
    await ensureDir(`${temp}/_docs/intent/Core/schema-marker`);
    await ensureDir(`${temp}/_docs/qa/Core/schema-marker`);
    await ensureDir(`${temp}/_docs/guide/Core`);
    await write(
      `${temp}/_docs/intent/Core/schema-marker/decision.md`,
      document("intent_schema: 2"),
    );
    await write(
      `${temp}/_docs/qa/Core/schema-marker/test-plan.md`,
      document("qa_status: planned\nrisk: Low\nqa_schema: 2"),
    );
    await write(
      `${temp}/_docs/intent/Core/schema-marker/invalid.md`,
      document('intent_schema: "2"'),
    );
    await write(
      `${temp}/_docs/guide/Core/unknown.md`,
      document("future_schema: 2"),
    );
    await write(
      `${temp}/_docs/guide/Core/duplicate.md`,
      `${frontMatter("")}\n${frontMatter("")}\n# Schema marker fixture\n`,
    );
    ...
    const intent = await run(
      "_docs/intent/Core/schema-marker/decision.md",
    );
    const qa = await run("_docs/qa/Core/schema-marker/test-plan.md");
    const invalid = await run(
      "_docs/intent/Core/schema-marker/invalid.md",
    );
    const unknown = await run("_docs/guide/Core/unknown.md");
    const duplicate = await run("_docs/guide/Core/duplicate.md");

    return intent.code === 0 &&
      !intent.stderr.includes("unknown field: intent_schema") &&
      qa.code === 0 &&
      !qa.stderr.includes("unknown field: qa_schema") &&
      invalid.code !== 0 &&
      invalid.stderr.includes("intent_schema must be the integer 2") &&
      unknown.code === 0 &&
      unknown.stderr.includes("unknown field: future_schema") &&
      duplicate.code !== 0 &&
      duplicate.stderr.includes("consecutive duplicate front matter blocks");
  } finally {
...
ok = await (async () => {
  const passed = await runFrontmatterSchemaMarkerCases();
  if (passed) {
    console.log(
      "PASS frontmatter schema markers, unknown-field warnings, and duplicate-block rejection",
    );
    return true;
  }
  console.error(
    "FAIL frontmatter schema markers, unknown-field warnings, and duplicate-block rejection",
  );
  return false;
})() && ok;
